    ...
    export class FurMaterialTool extends AbstractMaterialTool<FurMaterial> {
        ...
        */
        ...
        public update(): boolean {
            ...
            furFolder.add(this.material, "furLength").min(0).step(0.01).name("Fur Length").onChange((result: any) => { callback(); });
            furFolder.add(this.material, "furAngle").min(0).step(0.01).name("Fur Angle").onChange((result: any) => { callback(); });
            furFolder.add(this.material, "furSpacing").min(0).step(0.01).name("Fur Spacing").onChange((result: any) => { callback(); });
            furFolder.add(this.material, "furSpeed").min(0).max(1000).step(0.01).name("Fur Speed").onChange((result: any) => { callback(); });
            furFolder.add(this.material, "furDensity").min(0).step(0.01).name("Fur Density").onChange((result: any) => { callback(); });
