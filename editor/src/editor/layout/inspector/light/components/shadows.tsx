...
export class EditorLightShadowsInspector extends Component<IEditorLightShadowsInspectorProps, IEditorLightShadowsInspectorState> {
	...
	public render(): ReactNode {
		if (this.props.editor.layout.preview.clusteredLightContainer.lights.includes(this.props.light)) {
			return (
				<EditorInspectorSectionField title="Shadows">
					<div className="text-center text-muted-foreground">Shadows are not supported for lights in a clustered light container.</div>
	...
	private _createShadowGenerator(type: "none" | "classic" | "cascaded"): void {
		...
		if (type === "none") {
			this.props.onShadowGeneratorChanged();
