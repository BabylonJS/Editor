...
import { IAssetComponent, AssetElement, IAssetFile, IAssetExportConfiguration } from '../typings/asset';
...
export default class CodeExtension extends Extension<BehaviorMetadata> implements IAssetComponent {
    ...
    public async onSerializeFinalFiles (configuration: IAssetExportConfiguration): Promise<IAssetFile[]> {
        ...
        const data = this.onSerialize();
        const files = data.scripts.map(s => {
    ...
    public getConstructor (code: BehaviorCode, node: any, evaluate?: boolean): any {
        ...
        if (evaluate) {
            ...
            return window['EDITOR'].BehaviorCode.Constructors[fnName](this.scene, node, Extensions.Tools, Extensions.Mobile, editorRequire);
        }
        // Create script tag
        else {
