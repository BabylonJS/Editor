...
export default class PBRSpecularGlossinessMaterialTool extends MaterialTool<PBRSpecularGlossinessMaterial> {
	...
	*/
    public update(object: any): void {
        ...
        this.tool.addTexture(diffuseColor, this.editor, this.editor.core.scene, 'diffuseTexture', this.object, false).name('Texture');

        // Bump
        const normal = this.tool.addFolder('Normal');
        normal.open();
        this.tool.addTexture(normal, this.editor, this.editor.core.scene, 'normalTexture', this.object).name('Normal Texture');
        ...
        reflection.open();
        this.tool.addTexture(reflection, this.editor, this.editor.core.scene, 'environmentTexture', this.object, true, false).name('Environment Texture');

        // Specular Roughness
        const glossiness = this.tool.addFolder('Glossiness');
        glossiness.open();
        glossiness.add(this.object, 'glossiness').step(0.01).name('Glossiness');
        this.tool.addTexture(glossiness, this.editor, this.editor.core.scene, 'specularGlossinessTexture', this.object).name('Specular Glossiness Texture');
        ...
        this.tool.addTexture(occlusion, this.editor, this.editor.core.scene, 'occlusionTexture', this.object).name('Occlusion Texture');
