...
export default class GeometryWorker {
    ...
    public async writeIncremental(mesh: any, geometry: any, geometryPath: string): Promise<any> {
        const exists = this._savedFiles.includes(geometryPath);
        const stream = exists ? null : createWriteStream(geometryPath);
        ...
        if (!exists) {
            this._savedFiles.push(geometryPath);
        }
