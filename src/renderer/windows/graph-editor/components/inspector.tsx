...
export class Inspector extends React.Component<IInspectorProps> {
    ...
    public async setNode(node: GraphNode): Promise<void> {
        ...
        colors.open();
        colors.addColor(node, "bgcolor").name("Background Color").onChange(() => node.graph?.setDirtyCanvas(true, true));
        colors.addColor(node, "boxcolor").name("Box Color").onChange(() => node.graph?.setDirtyCanvas(true, true));
        ...
        for (const p in node.properties) {
            ...
            if (widget?.options?.values) {
                ...
                properties.addSuggest(node.properties, p, values).name(this._getFormatedname(p));
                continue;
            }
            ...
            switch (ctor) {
                ...
                case "string":
                case "boolean":
                    controller = properties.add(node.properties, p).onChange(() => this.props.editor.graph.graphCanvas?.setDirty(true, true)).name(this._getFormatedname(p));
                    break;
                case "vector2":
                case "vector3":
